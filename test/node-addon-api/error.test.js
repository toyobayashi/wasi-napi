...
async function test (bindingPath) {
  ...
  const p = require('./napi_child').spawnSync(
    process.execPath, ['--no-warnings', '--expose-gc', '--experimental-wasi-unstable-preview1', ...(process.env.MEMORY64 ? ['--experimental-wasm-memory64'] : []), __filename, 'fatal', bindingPath])
